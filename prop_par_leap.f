...
C     
     ...
     6 XS, VS, XL, VL, PENS, PENL, pmass)
        ENL = TE + PE
        VIRL = TE/PE
       WRITE(44,*) T, RERR_C, RERR_V, RERR_E, RERR_P,
     6 ENL, VIRL
c
c
      COUNTER = 0
...
C
        tt = t
       WRITE(*,*) Tt, RERR_C, RERR_V, RERR_E, RERR_P,
     6 ENL, VIRL       
c
C
...
c        DO JJ = 1, n ! II +1, N
c        dfac= 1.0
c        if (jj .lt. ii + 1) dfac =0.0 
c      DX = X (II, 1) - X (JJ, 1)
c      DY = X (II, 2) - X (JJ, 2)
c      DZ = X (II, 3) - X (JJ, 3)   
C
c      DIST2 = DX* DX + DY* DY + DZ * DZ
c      DIST = DSQRT (DIST2+0.0005)
c      COFFI = 1d0/(DIST * DIST * DIST)
c      DISTI = 1D0/DIST
c      PE = PE - PMASS * PMASS * DISTI * dfac
C
c      A(JJ, 1) = A(JJ, 1) + (PMASS*COFFI)* DX *dfac
c      A(JJ, 2) = A(JJ, 2) + (PMASS*COFFI)* DY *dfac
c      A(JJ, 3) = A(JJ, 3) + (PMASS*COFFI)* DZ *dfac
C
c      A(II, 1) = A(II, 1) - (PMASS*COFFI)* DX * dfac
c      A(II, 2) = A(II, 2) - (PMASS*COFFI)* DY * dfac
c      A(II, 3) = A(II, 3) - (PMASS*COFFI)* DZ * dfac
C
c      PEN (II) = PEN (II) - PMASS * PMASS * DISTI * dfac
c      PEN (JJ) = PEN (JJ) - PMASS * PMASS * DISTI * dfac
c
c        END DO
c      END DO
